...
export class RemoteUsersService {
	...
	RemoteUser_GetAudioActive(connectionId:string){
				return this.getRemoteUserByConnectionId(connectionId).isAudioActive();
			}
	RemoteUser_IsWebCamEnabled(connectionId:string): boolean {
		return this.getRemoteUserByConnectionId(connectionId).isCamera();
	}
	RemoteUser_GetWebcamPublisher(connectionId:string): Subscriber {
		return <Subscriber>this.getRemoteUserByConnectionId(connectionId).getStreamManager();
	}
	RemoteUser_GetRemoteUserList(){
