...
export class VideoRoomComponent implements OnInit, OnDestroy {
	...
	@ViewChild('participantComponent') participantComponent: ChatComponent;
	@ViewChild('sidenavi') participantSidenav: MatSidenav;
	...
	private startRecordingRes: string = "";
	private stopRecordingId: string = "";
	constructor(
	...
	onConfigRoomJoin() {
		...
		let oIsMuted:any = sessionStorage.getItem('oIsMuted');
		if (oIsMuted == 1) {
	...
	async joinToSession() {
		...
		this.chatService.setParticipantComponent(this.participantSidenav);
	...
	async leaveSession() {
		...
		localStorage.setItem("SessionId", this.session.sessionId);
	...
	async startRecording()
  {
		...
		this.log.d('start Recording..' + this.session.sessionId);
		this.startRecordingRes == "";
		const res = await this.networkService.startRecording(this.session.sessionId, "", "");
		this.startRecordingRes = res['id'];
		this.showPlayBtn = false;
		console.log(this.startRecordingRes);
	}

	async stopRecording() {
		this.log.d('stop Recording..');
		this.log.d('stop Recording..' + this.session.sessionId);
		this.stopRecordingId == ""
		let emailId = sessionStorage.getItem("email");
		const res = await this.networkService.stopRecording(this.startRecordingRes, emailId);
		this.showPlayBtn = true;
		this.log.d('stop Recording response ' + this.stopRecordingId);
	}
	...
	async toggleMic() {
		this.localUsersService.LocalUserService_MuteUserHimSelf(true);
		...
		console.log('response of is mute all',oResponse);
		if (oResponse) {
			if (oResponse.IsMuted == 1) {
				if (oResponse.OwnerEmail == this.tokenService.Token_GetEmail()) {
					if (this.localUsersService.isWebCamEnabled()) {
				...
				if(!this.localUsersService.hasWebcamAudioActive()){
						this.localUsersService.LocalUserService_MuteUserHimSelf(true);
				}
				else{
					this.localUsersService.LocalUserService_MuteUserHimSelf(false);
				}
				if(JSON.parse(sessionStorage.getItem('userMuteedHimself'))){
					sessionStorage.setItem('userMuteedHimself', JSON.stringify(false));
				}else{
					sessionStorage.setItem('userMuteedHimself', JSON.stringify(true));
				}
				if (this.localUsersService.isWebCamEnabled()) {
	...
	async VideoRoom_ToggleMicAll() {
		let rUsers = this.remoteUsersService.RemoteUser_GetRemoteUserList();
		if (rUsers) {
			let nMuteValue;
			...
			rUsers.forEach((value) => {
				value.forEach((element) => {
					this.remoteUsersService.RemoteUser_IsWebCamEnabled(element['connectionId']);
					this.openViduWebRTCService.publishWebcamAudioRemote(!this.muteAll, element['connectionId']);
				});
			});
			let sSessionID = this.tokenService.getSessionId();
			let MuteAllResponse = await this.networkService.NetworkService_MuteAll(sSessionID, nMuteValue);
		}
	}
	async toggleCam() {
	...
	private subscribeToRemoteUsers() {
		...
		this.remoteUserNameSubscription = this.remoteUsersService.remoteUserNameList.subscribe((names: UserName[]) => {
			...
			console.log('this.C',this.participantsNameList);
			this.tokenService.Token_AddParticipant(this.participantsNameList.slice());
		});
	}

	toolbarMicIconEnabledAll(): boolean {
		return this.muteAll;
	}
}
